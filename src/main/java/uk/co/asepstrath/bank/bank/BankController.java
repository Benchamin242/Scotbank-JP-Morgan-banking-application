...
public class BankController {
   ...
   @GET ("/viewAllAccounts")
    ...
    @GET("/sanctioned")
    public ModelAndView generateTransactionReport(Session session) {
        try(Connection connection = dataSource.getConnection()) {
            // Query to calculate total amount spent in sanctioned businesses
            String sanctionedQuery = "SELECT SUM(th.amount) AS sanctioned_amount " +
                    "FROM transactionHistory th " +
                    "INNER JOIN businessDetails bd ON th.to = bd.name " +
                    "WHERE bd.sanctioned = true";

            // Query to calculate total amount spent in non-sanctioned businesses
            String nonSanctionedQuery = "SELECT SUM(th.amount) AS non_sanctioned_amount " +
                    "FROM transactionHistory th " +
                    "INNER JOIN businessDetails bd ON th.to = bd.name " +
                    "WHERE bd.sanctioned = false";

            // Execute the sanctioned query
            PreparedStatement sanctionedStmt = connection.prepareStatement(sanctionedQuery);
            ResultSet sanctionedResult = sanctionedStmt.executeQuery();

            double sanctionedAmount = 0.0;
            if (sanctionedResult.next()) {
                sanctionedAmount = sanctionedResult.getDouble("sanctioned_amount");
            }

            // Execute the non-sanctioned query
            PreparedStatement nonSanctionedStmt = connection.prepareStatement(nonSanctionedQuery);
            ResultSet nonSanctionedResult = nonSanctionedStmt.executeQuery();

            double nonSanctionedAmount = 0.0;
            if (nonSanctionedResult.next()) {
                nonSanctionedAmount = nonSanctionedResult.getDouble("non_sanctioned_amount");
            }

            // Prepare data to pass to the HBS template
            Map<String, Object> model = new HashMap<>();
            model.put("sanctionedAmount", sanctionedAmount);
            model.put("nonSanctionedAmount", nonSanctionedAmount);

            // Return the ModelAndView with the data and the template name
            return new ModelAndView("TransactionReport.hbs", model);

        } catch (SQLException e) {
            throw new StatusCodeException(StatusCode.SERVER_ERROR, "Error generating transaction report", e);
        }}
    @GET("/summary")
